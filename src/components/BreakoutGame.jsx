        ...
        {gameState === 'menu' && (
          <div className="absolute inset-0 bg-slate-950/95 backdrop-blur-md flex flex-col justify-between overflow-hidden z-30">
            ...
            <div className="w-full bg-slate-950/90 border-t border-slate-800/80 p-2.5 sm:p-3 flex items-center justify-center backdrop-blur shrink-0 z-10">
              <button
                onClick={startGame}
                className="w-full max-w-sm py-2.5 sm:py-3 bg-gradient-to-r from-amber-500 to-pink-500 hover:from-amber-400 hover:to-pink-400 text-slate-950 font-extrabold rounded-xl shadow-lg shadow-amber-500/30 text-sm sm:text-base transition-transform hover:scale-102 active:scale-95 flex items-center justify-center gap-2"
              >
                <Play className="w-4 h-4 sm:w-5 sm:h-5 fill-current" /> START GAME
              </button>
            </div>
          </div>
        )}
