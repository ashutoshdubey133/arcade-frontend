...
import {
  ...
  Flame, Shield, Play, Keyboard, Zap, Trophy, Star
} from 'lucide-react';
...
const getDifficultyParams = (elapsed, wordsDestroyed) => {
  const fallSpeed = Math.min(5.0, 0.85 + Math.log(elapsed / 12 + 1) * 1.1 + wordsDestroyed * 0.012);
...
const pickWord = (weights) => {
  ...
  for (const [len, w] of entries) {
    r -= w;
    if (r <= 0) { const pool = WORD_POOLS[+len] || WORD_POOLS[3]; return pool[Math.floor(Math.random() * pool.length)]; }
  }
...
export default function TypingGame({ onBackToHub, onSaveScore }) {
  ...
  const engineState = useRef({ meteors: [], lasers: [], particles: [], stars: [], screenShake: 0, activeTargetId: null, pendingPowerup: null });

  useEffect(() => {
    const stars = [];
    for (let i = 0; i < 60; i++) stars.push({ x: Math.random()*CANVAS_WIDTH, y: Math.random()*(CANVAS_HEIGHT*0.6), radius: Math.random()*1.5+0.5, alpha: Math.random()*0.8+0.2, twinkleSpeed: 0.02+Math.random()*0.03 });
    engineState.current.stars = stars;
  }, []);

  const focusMobileKeyboard = () => { if (gameState === 'playing' && mobileInputRef.current) mobileInputRef.current.focus(); };
  useEffect(() => { if (gameState === 'playing') setTimeout(() => mobileInputRef.current?.focus(), 100); }, [gameState]);
  const handleMobileInputChange = (e) => { const val = e.target.value; if (!val) return; const lastChar = val.slice(-1).toUpperCase(); if (/^[A-Z]$/.test(lastChar)) processKeyPress(lastChar); e.target.value = ''; };
  const handleToggleMute = () => setIsMuted(soundFX.toggleMute());

  const addParticles = (x, y, color, count = 12) => {
    for (let i = 0; i < count; i++) { const angle = Math.random()*Math.PI*2; const speed = 1+Math.random()*5; engineState.current.particles.push({ x, y, vx: Math.cos(angle)*speed, vy: Math.sin(angle)*speed, radius: 2+Math.random()*4, alpha: 1, color }); }
  };

  const applyPowerup = (powerupId) => {
    const now = Date.now(); const engine = engineState.current;
    soundFX.playPowerUp(); confetti({ particleCount: 35, spread: 55, origin: { y: 0.6 }, zIndex: 9999 });
    switch (powerupId) {
      case 'shield': setHealth(h => Math.min(100, h + 30)); break;
      case 'slow':   slowExpiry.current = now+8000; setActivePowerups(p => [...p.filter(x => x.id!=='slow'), { id:'slow', badgeLabel:'SLOW WAVE', color:'#38bdf8', expiresAt:now+8000 }]); break;
      case 'blast':  engine.meteors.forEach(m => addParticles(m.x,m.y,m.color,10)); engine.meteors=[]; engine.activeTargetId=null; setScore(s => s+150*scoreMultRef.current); confetti({ particleCount:80, spread:100, origin:{y:0.5} }); break;
      case 'double': scoreMultRef.current=2; multiplierExpiry.current=now+10000; setHasDouble(true); setActivePowerups(p => [...p.filter(x => x.id!=='double'), { id:'double', badgeLabel:'2x SCORE', color:'#a855f7', expiresAt:now+10000 }]); break;
      case 'freeze': freezeExpiry.current=now+5000; setActivePowerups(p => [...p.filter(x => x.id!=='freeze'), { id:'freeze', badgeLabel:'FROZEN', color:'#06b6d4', expiresAt:now+5000 }]); break;
      default: break;
    }
  };
    ...
    if (target) {
      ...
      if (target.text[target.typedIndex] === typedChar) {
        ...
        if (target.typedIndex >= target.text.length) {
          ...
          setTotalWords(w => w+1);
          setCombo(c => { const next=c+1; setScore(s => s+(target.text.length*20+next*5)*scoreMultRef.current); return next; });
          if (wordsDestroyedRef.current >= nextPowerupAt.current) { nextPowerupAt.current = wordsDestroyedRef.current+5+Math.floor(Math.random()*8); engine.pendingPowerup = POWERUP_DEFS[Math.floor(Math.random()*POWERUP_DEFS.length)]; }
        }
      } else { soundFX.playTypeMiss(); setCombo(0); engine.activeTargetId = null; }
      setLiveAccuracy(Math.round((lettersCorrect.current / lettersAttempted.current) * 100));
    } else { soundFX.playTypeMiss(); setCombo(0); }
  };

  useEffect(() => {
    const handleKeyDown = (e) => { if (gameState !== 'playing') return; if (e.key.length !== 1) return; const c = e.key.toUpperCase(); if (!/^[A-Z]$/.test(c)) return; processKeyPress(c); };
    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [gameState]); // eslint-disable-line react-hooks/exhaustive-deps

  const startGame = () => {
    setScore(0); setHealth(100); setCombo(0); setTotalWords(0); setScoreSubmitted(false);
    setActivePowerups([]); setSurvivalTime(0); setHasDouble(false); setLiveAccuracy(100); setFinalAccuracy(100);
    lettersAttempted.current=0; lettersCorrect.current=0; wordsDestroyedRef.current=0;
    nextPowerupAt.current=5+Math.floor(Math.random()*5); scoreMultRef.current=1;
    multiplierExpiry.current=0; slowExpiry.current=0; freezeExpiry.current=0;
    const engine = engineState.current;
    engine.meteors=[]; engine.lasers=[]; engine.particles=[]; engine.screenShake=0; engine.activeTargetId=null; engine.pendingPowerup=null;
    gameStartTime.current=Date.now(); lastSpawnTime.current=Date.now();
  ...
  useEffect(() => {
    ...
    const updateAndDraw = () => {
      const engine = engineState.current; const now = Date.now(); const elapsed = (now-gameStartTime.current)/1000;
      if (multiplierExpiry.current > 0 && now > multiplierExpiry.current) { scoreMultRef.current=1; multiplierExpiry.current=0; setHasDouble(false); setActivePowerups(p => p.filter(x => x.id!=='double')); }
      if (slowExpiry.current > 0 && now > slowExpiry.current) { slowExpiry.current=0; setActivePowerups(p => p.filter(x => x.id!=='slow')); }
      if (freezeExpiry.current > 0 && now > freezeExpiry.current) { freezeExpiry.current=0; setActivePowerups(p => p.filter(x => x.id!=='freeze')); }
      const isFrozen = freezeExpiry.current > now; const slowFactor = slowExpiry.current > now ? 0.38 : 1.0;
      const params = getDifficultyParams(elapsed, wordsDestroyedRef.current);
      if (now - lastSpawnTime.current > params.spawnInterval) {
        lastSpawnTime.current = now;
        if (engine.pendingPowerup) { const pu=engine.pendingPowerup; engine.pendingPowerup=null; const word=POWERUP_WORD_POOL[Math.floor(Math.random()*POWERUP_WORD_POOL.length)]; engine.meteors.push({ id:Math.random().toString(), x:80+Math.random()*(CANVAS_WIDTH-160), y:-20, text:word, typedIndex:0, speed:Math.max(0.55,params.fallSpeed*0.55), color:pu.color, isPowerup:true, powerupId:pu.id, powerupIcon:POWERUP_CANVAS_ICON[pu.id]||'[P]' }); }
        else { const word=pickWord(params.weights); const colors=['#06b6d4','#a855f7','#f43f5e','#f59e0b','#10b981']; engine.meteors.push({ id:Math.random().toString(), x:60+Math.random()*(CANVAS_WIDTH-120), y:-20, text:word, typedIndex:0, speed:params.fallSpeed+(Math.random()*0.28-0.14), color:colors[Math.floor(Math.random()*colors.length)], isPowerup:false }); }
      }
      if (!isFrozen) {
        for (let i = engine.meteors.length-1; i >= 0; i--) {
          const m = engine.meteors[i]; m.y += m.speed*slowFactor;
          if (Math.random() < 0.28) engine.particles.push({ x:m.x+(Math.random()*16-8), y:m.y-8, vx:Math.random()*0.7-0.35, vy:-1-Math.random()*0.8, radius:2, alpha:0.55, color:m.color });
          if (m.y >= CANVAS_HEIGHT-70) {
            soundFX.playExplosion(); addParticles(m.x, CANVAS_HEIGHT-60, '#ef4444', 18); engine.screenShake=12;
            if (engine.activeTargetId === m.id) engine.activeTargetId=null;
            engine.meteors.splice(i,1); setCombo(0);
            setHealth(h => { const dmg=m.isPowerup?5:15; const nextH=Math.max(0,h-dmg); if(nextH<=0){ soundFX.playGameOver(false); const fa=lettersAttempted.current>0?Math.round((lettersCorrect.current/lettersAttempted.current)*100):100; setFinalAccuracy(fa); setGameState('gameover'); } return nextH; });
          }
        }
      }
      ctx.save();
      if (engine.screenShake > 0) { ctx.translate((Math.random()-0.5)*engine.screenShake,(Math.random()-0.5)*engine.screenShake); engine.screenShake*=0.83; if(engine.screenShake<0.5) engine.screenShake=0; }
      ctx.clearRect(0,0,CANVAS_WIDTH,CANVAS_HEIGHT);
      const skyGrad=ctx.createLinearGradient(0,0,0,CANVAS_HEIGHT); skyGrad.addColorStop(0,'#0a0a1a'); skyGrad.addColorStop(0.5,'#1e112a'); skyGrad.addColorStop(1,'#2d123d'); ctx.fillStyle=skyGrad; ctx.fillRect(0,0,CANVAS_WIDTH,CANVAS_HEIGHT);
      ...
      engine.stars.forEach(st => { st.alpha+=st.twinkleSpeed; if(st.alpha>1||st.alpha<0.2) st.twinkleSpeed=-st.twinkleSpeed; ctx.save(); ctx.globalAlpha=Math.max(0.1,st.alpha); ctx.fillStyle='#fff'; ctx.beginPath(); ctx.arc(st.x,st.y,st.radius,0,Math.PI*2); ctx.fill(); ctx.restore(); });
      ctx.save(); ctx.shadowColor='#c084fc'; ctx.shadowBlur=25; ctx.fillStyle='#f3e8ff'; ctx.beginPath(); ctx.arc(680,80,35,0,Math.PI*2); ctx.fill(); ctx.restore();
      ...
      ctx.save(); ctx.shadowColor='#38bdf8'; ctx.shadowBlur=15; ctx.fillStyle='#0284c7'; ctx.beginPath(); ctx.arc(CANVAS_WIDTH/2,CANVAS_HEIGHT-10,24,Math.PI,0,false); ctx.fill(); ctx.restore();
      ctx.save(); ctx.globalAlpha=0.4; ctx.font='bold 11px monospace'; ctx.fillStyle='#94a3b8'; ctx.textAlign='right'; ctx.textBaseline='bottom'; ctx.fillText(Math.floor(elapsed)+'s survived', CANVAS_WIDTH-8, CANVAS_HEIGHT-6); ctx.restore();
      for (let i=engine.lasers.length-1; i>=0; i--) { const l=engine.lasers[i]; l.alpha-=0.08; if(l.alpha<=0){engine.lasers.splice(i,1);continue;} ctx.save(); ctx.globalAlpha=l.alpha; ctx.strokeStyle=l.color; ctx.lineWidth=4; ctx.shadowColor=l.color; ctx.shadowBlur=12; ctx.beginPath(); ctx.moveTo(l.x1,l.y1); ctx.lineTo(l.x2,l.y2); ctx.stroke(); ctx.restore(); }
      for (let i=engine.particles.length-1; i>=0; i--) { const pt=engine.particles[i]; pt.x+=pt.vx; pt.y+=pt.vy; pt.alpha-=0.03; if(pt.alpha<=0){engine.particles.splice(i,1);continue;} ctx.save(); ctx.globalAlpha=pt.alpha; ctx.fillStyle=pt.color; ctx.beginPath(); ctx.arc(pt.x,pt.y,pt.radius,0,Math.PI*2); ctx.fill(); ctx.restore(); }
      for (const m of engine.meteors) {
        ctx.save(); const isTarget=engine.activeTargetId===m.id;
        if (m.isPowerup) { const pulse=Math.sin(now/220)*0.45+0.55; ctx.shadowColor=m.color; ctx.shadowBlur=18+pulse*18; ctx.fillStyle='#0d1f14'; ctx.strokeStyle=m.color; ctx.lineWidth=2.5; }
        else { ctx.shadowColor=isTarget?'#22c55e':m.color; ctx.shadowBlur=isTarget?22:12; ctx.fillStyle=isTarget?'#052e16':'#0f172a'; ctx.strokeStyle=isTarget?'#22c55e':m.color; ctx.lineWidth=isTarget?3:2; }
        const iconPad=m.isPowerup?28:0; const boxWidth=Math.max(52,m.text.length*18+20+iconPad); const boxHeight=m.isPowerup?44:36;
        ctx.beginPath(); ctx.roundRect(m.x-boxWidth/2, m.y-boxHeight/2, boxWidth, boxHeight, 10); ctx.fill(); ctx.stroke();
        if (m.isPowerup) { ctx.font='bold 10px monospace'; ctx.textAlign='left'; ctx.textBaseline='middle'; ctx.fillStyle=m.color; ctx.shadowBlur=0; ctx.fillText(m.powerupIcon, m.x-boxWidth/2+4, m.y); }
        ctx.font='bold 18px monospace'; ctx.textAlign='left'; ctx.textBaseline='middle';
        const typed=m.text.substring(0,m.typedIndex); const remaining=m.text.substring(m.typedIndex);
        const totalW=ctx.measureText(m.text).width; let startX=m.x-totalW/2+(m.isPowerup?iconPad/2:0);
  ...
  const handleSubmitScore = () => {
    if (!scoreSubmitted && onSaveScore) {
      onSaveScore({ playerName: playerName||'Player 1', game:'Sky Letters', score, mode: survivalTime+'s | '+totalWords+' words | '+finalAccuracy+'% acc', date: new Date().toISOString() });
    ...
    <div className="flex flex-col items-center justify-center min-h-[85vh] p-4">
      <div className="w-full max-w-[800px] flex items-center justify-between mb-3 bg-slate-900/80 p-3 rounded-xl border border-slate-800 backdrop-blur">
        ...
        <div className="flex items-center gap-4 font-mono font-bold text-sm">
          ...
          {combo > 1 && <div className="text-amber-400 flex items-center gap-1 animate-bounce text-xs"><Flame className="w-3.5 h-3.5" /> {combo}x</div>}
        </div>
      ...
      {activePowerups.length > 0 && (
        <div className="w-full max-w-[800px] flex gap-2 mb-2 flex-wrap">
          {activePowerups.map(pu => {
            ...
            return (
              <div key={pu.id} className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-xs font-bold font-mono"
      ...
      <div className="relative border-4 border-slate-800 rounded-2xl overflow-hidden shadow-2xl shadow-indigo-950/40 bg-slate-950 w-full max-w-[800px] min-h-[420px] flex items-center justify-center">
            ...
            <div className="w-full max-w-sm mx-auto flex flex-col items-center gap-3 pt-2">
              ...
              <div className="w-full bg-slate-900/80 rounded-xl border border-slate-800 p-3 text-left">
                ...
                <div className="grid grid-cols-1 gap-1.5">
                  {POWERUP_DEFS.map(pu => (
                    <div key={pu.id} className="flex items-center gap-2 text-xs">
                      <span className="font-bold w-16 shrink-0" style={{ color: pu.color }}>{pu.label}</span>
        ...
        {gameState === 'gameover' && (
          <div className="absolute inset-0 bg-slate-950/92 backdrop-blur-md flex flex-col items-center justify-center p-6 text-center z-30">
            ...
            <p className="text-slate-500 text-xs mb-5">Your run has ended — here are your stats:</p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6 w-full max-w-md">
              <div className="bg-slate-900 border border-cyan-900/50 rounded-xl py-4 px-2 text-center">
                <div className="text-2xl font-extrabold text-cyan-400 font-mono">{survivalTime}s</div>
                <div className="text-slate-500 text-[10px] mt-1 uppercase tracking-widest">Survived</div>
              </div>
              <div className="bg-slate-900 border border-purple-900/50 rounded-xl py-4 px-2 text-center">
                <div className="text-2xl font-extrabold text-purple-400 font-mono">{score.toLocaleString()}</div>
                <div className="text-slate-500 text-[10px] mt-1 uppercase tracking-widest">Score</div>
              </div>
              <div className="bg-slate-900 border border-emerald-900/50 rounded-xl py-4 px-2 text-center">
                <div className="text-2xl font-extrabold text-emerald-400 font-mono">{totalWords}</div>
                <div className="text-slate-500 text-[10px] mt-1 uppercase tracking-widest">Words</div>
              </div>
              <div className="bg-slate-900 border border-slate-700 rounded-xl py-4 px-2 text-center">
                <div className={"text-2xl font-extrabold font-mono " + finalAccColor}>{finalAccuracy}%</div>
                <div className="text-slate-500 text-[10px] mt-1 uppercase tracking-widest">Accuracy</div>
              </div>
            </div>
      ...
      <div className="w-full max-w-[800px] mt-4 p-3 bg-slate-900/60 rounded-xl border border-slate-800 text-xs text-slate-400 flex flex-wrap items-center justify-between gap-4">
        ...
        <div><strong className="text-purple-400">Powerups:</strong> Glowing meteors grant special abilities — type them fast!</div>
      </div>
    </div>
  );
}
